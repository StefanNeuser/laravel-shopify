...
class ShopifyAPI implements LoggerAwareInterface
{
     ...
     * @return self
     */
    public function withSession(string $shopDomain, string $accessToken, Closure $closure)
    {
        $this->log("WithSession started for {$shopDomain}");

        // Clone the API class and bind it to the closure
        $clonedApi = (clone $this)->setShopDomain($shopDomain)->setAccessToken($accessToken);
    ...
    public function getBaseUri()
    {
        ...
        return new Uri("https://{$this->shopDomain}");
    }
